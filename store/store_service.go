...
// LongUrl retrieves the original URL from the Redis cache, given the shortened URL
func LongUrl(shortUrl string) string {
	result, err := storeService.redisClient.Get(ctx, shortUrl).Result()
	if err != nil {
		panic(fmt.Sprintf("Failed LongUrl url | Error: %v - shortUrl: %s\n", err, shortUrl))
	}
	return result
}
